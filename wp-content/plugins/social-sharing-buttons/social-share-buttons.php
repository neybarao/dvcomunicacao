...
function zb_sb_setting(){
	...
	if(isset($access_key) && !empty($access_key) && strlen($access_key) > 0 && $ztb_status_disconnect == $connected){
		...
		$button = '<a  target="zotabox" href="'.$domain_action.'/customer/access/PluginLogin/?customer='.$ztb_id.'&access='.$access_key.'&app=sb&platform=wordpress&utm_source='.str_replace('http://', '', home_url()) .'">
